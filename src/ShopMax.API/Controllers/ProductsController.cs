...
namespace ShopMax.API.Controllers
{
	...
	public class ProductsController : BaseController
	{
		...
		[AllowAnonymous]
		[HttpGet("products-from-seller/")]
		[ProducesResponseType(StatusCodes.Status200OK)]
		[ProducesResponseType(StatusCodes.Status401Unauthorized)]
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		[ProducesDefaultResponseType]
		public async Task<ActionResult<IEnumerable<Product>>> GetProductsFromSeller()
		{
			var id = GetUserId();

			if (id <= 0) return Unauthorized("Claim 'id' not found.");

			var products = await _productService.GetProductsFromSeller(id);
